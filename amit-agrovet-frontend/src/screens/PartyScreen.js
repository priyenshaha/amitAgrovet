...
const PartyScreen = (props) => {
  ...
  const onConfirmParty = () => { 
    ...
    gstNumber.length>0 && gstNumber.length!==12 ? setGstNumberError("GST number should have 12 characters") : setGstNumberError("")
    type.length<1 ? setTypeError("Select type") : setTypeError("")
    
    // make req
    if(!(name.length<1 || (gstNumber.length>0 && gstNumber.length!==12) || type.length<1)){
      setLoading(true)
                ...
                data.map(party => {
                  ...
                  return (
                    <tr key={party.id} onChange={() => {party.edited=true; setEditFlag(!editFlag)}}>
                      <td>{party.id}</td>
                      <td><input className="text-center" style={{width: "10rem"}} type="text" value={party.name} onChange={e => {party.name = e.target.value}}/></td>
                      <td><input className="text-center" style={{width: "10rem"}}type="text" value={party.address ? party.address : ""} onChange={e => {party.address = e.target.value}}/></td>
                      <td><input className="text-center" style={{width: "3rem"}} type="text" value={party.stateCode ? party.stateCode : ""} onChange={e => {party.stateCode = e.target.value}}/></td>
                      <td><input className="text-center" style={{width: "8rem"}} type="text" value={party.gstNumber ? party.gstNumber : ""} onChange={e => {party.gstNumber = e.target.value}} /></td>
                      <td><input className="text-center" style={{width: "6rem"}} type="text" value={party.price ? party.price : ""} onChange={e => {party.price = e.target.value}} /></td>
                      <td>
                        <select className='form-control' value={party.type} onChange={e => {party.type = e.target.value}}>
